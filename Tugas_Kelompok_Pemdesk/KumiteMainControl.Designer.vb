...
Partial Public Class KumiteMainControl
    ...
    Private Sub BuildAthletePanel(p As Panel, side As String, clr As Color, n As String, t As String, info As String, scoreVal As String)
        ...
        AddHandler pic.Paint, Sub(s, e)
                                  ...
                                  Using pen As New Pen(Color.MediumPurple, 2)
                                      e.Graphics.DrawEllipse(pen, 2, 2, 65, 65)
                                  End Using
                              End Sub
        p.Controls.Add(pic)
    ...
    Private Sub BuildRightPanel(p As Panel)
        ...
        numWaitMin = New NumericUpDown With {.Bounds = New Rectangle(10, 280, 40, 25), .Value = 2, .Maximum = 59}
        p.Controls.Add(numWaitMin)
        p.Controls.Add(New Label With {.Text = ":", .Bounds = New Rectangle(52, 282, 10, 20), .Font = FONT_BOLD})
        numWaitSec = New NumericUpDown With {.Bounds = New Rectangle(65, 280, 40, 25), .Value = 0, .Maximum = 59}
        p.Controls.Add(numWaitSec)
        btnStartWaitTimer = CreateBtn("Start", 120, 278, 95, 28, COLOR_GOLD, COLOR_TEXT_DARK, FONT_BOLD)
        ...
        Dim btnStartScoreboard As Button = CreateBtn("Start Scoreboard  📊", 45, 460, 175, 30, Color.PaleGreen, COLOR_TEXT_DARK)
    ...
    Private Sub BuildFooter(p As Panel)
        ...
        ' -- UI HANTEI --
        btnHantei = CreateBtn("Hantei 🚩", 460, 8, 80, 40, Color.White, Color.Black, FONT_BOLD)
